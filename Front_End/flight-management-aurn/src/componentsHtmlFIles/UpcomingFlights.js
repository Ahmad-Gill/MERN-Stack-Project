...
import { handlePaymentDetails } from '../componentsHtmlFIles/card';
...
const visibleFields = [
    ...
    'seatPreference', 'seatCount', 'mealPreference', 'totalAmount', "paymentConfirmed"
];
const headerMap = {
...
<div className="analytics-container">
  ...
  onClick={async () => {
    if (row[key]) {
      try {
        setIsLoading(true);
        const response = await fetch("http://localhost:5000/flight/email-ticket", {
          method: "POST",
          headers: {
        ...
        if (result.success) {
          setPopupMessage("Ticket generated successfully check your Email!");
        ...
        setPopupMessage("Server error while generating ticket.");
        setPopupType("error");
        setIsLoading(false);
      }
    } else {
...
>
  {row[key] ? "Get Ticket" : "Not Confirmed"}
</Button>
